...
- [ ] 1.1 Update `CONTEXT.md` so `Wide hero` replaces `Hero-on-left`, defines the left browser/right hero arrangement, and preserves `Inline hero`; verify the current presentation section contains no `Hero-on-left` term.
- [ ] 1.2 Add an ADR that supersedes ADR 0021 with the Wide hero/Inline hero decision and leaves accepted historical ADR and archived OpenSpec files unchanged; verify ADR links and supersession text resolve correctly.
