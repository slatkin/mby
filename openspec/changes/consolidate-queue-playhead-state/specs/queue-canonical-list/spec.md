...
The projection's semantic active state and `progress_percent` SHALL derive from one reconciled playback-position value, not from independently maintained fields. That value SHALL identify the active queue scope, the active slot, and whether the active slot is confirmed by the playback owner or is an optimistic prediction awaiting confirmation. An optimistic prediction SHALL record why it is optimistic: a queue edit relocated the still-playing item, or a different item was selected to play. Reconciliation against playback-owner status SHALL occur in one place, outside the render path, and SHALL clear a prediction once the owner's reported slot and queue length match it.

While a prediction says a different item was selected, the projection SHALL report that item's slot as active with no progress until reconciliation confirms it. While a prediction says the still-playing item was relocated, the projection SHALL keep the item's existing progress.
